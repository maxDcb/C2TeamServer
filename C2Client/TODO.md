...
| 18 | [ ] | Ajouter `ListCommands` / `ListModules` structure | L | Tres fort | Partiel. `ListCommands` expose un catalogue serveur, tab UI `Commands`, autocomplete console sans fallback hardcode, specs simples pour modules, `ListModules` stream les modules suivis par beacon, `loadModule` cache les modules actifs et `unloadModule` propose les modules charges. Reste aide detaillee et persistence/historique modules. |
